...
do
  ...
  TESTID="$TESTRUN/$i"
  # Warm up run
  echo "$TEST" | $CH_CLIENT -h $CH_HOST --port 9440 -s --user=$CH_USER --password=$CH_PASS --database=$CH_DB --log_queries=0 > /dev/null
  ...
  printf " "
  $CH_CLIENT -h $CH_HOST --port 9440 -s --user=$CH_USER --password=$CH_PASS --database=$CH_DB --log_queries=0 --multiquery \
    --query="SYSTEM FLUSH LOGS; SELECT round(sum(query_duration_ms)/1000/$TRIES,3) FROM system.query_log where event_date=today() and type='QueryFinish' and query like '%$TESTID%'"
done
$CH_CLIENT -h $CH_HOST --port 9440 -s --user=$CH_USER --password=$CH_PASS --database=$CH_DB --log_queries=0 \
  --query="SELECT 'Total :', round(sum(query_duration_ms)/1000/$TRIES,3) FROM system.query_log where event_date=today() and type='QueryFinish' and query like '%$TESTRUN%'"
